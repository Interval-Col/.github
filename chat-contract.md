...
| **CH6** | **UI copy in neutral-Colombian Spanish — register follows the ASSISTANT'S canon, not this clause.** Every user-facing chat string — launcher/aria labels, empty state, starters, thinking state, error and rate-limit notices — is in es-CO. ⚠️ **The `usted` this clause used to mandate was corrected on 2026-09-08**: Nerea tutea (`NEREA.md`) and Sol tutea too, with register by surface — neutral/technical about the catalogue, `usted` when the question touches legal matters (`SOL.md` §3). A clause that hard-codes one register silently reverts a persona ruling. *Prose clause — not machine-checked.* | RFC 0017 Phase 3; `SOL.md` §3; finance-lch `HelpChat.vue` |
