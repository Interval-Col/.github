...
- **Faro (pilot light)** #E4002B (constante de marca `#E4002B`); halo al latir; ritmo ligado a `<html data-status>`. → `registry/app/components/SystemBeacon.vue (place="topbar" + place="canvas", both mounted in layouts/default.vue) + app/plugins/beacon-mode.client.ts (publishes data-beacon) + app/plugins/health-beacon.client.ts (publishes data-status); pilot in AppLogo.vue + pharos-components.css (.pharos-pilot)`.
- **Beacon de estado · presentación** Barra (esquina) — la app escoge el registro, no el estado: `<html data-beacon>` selecciona entre punto/etiqueta/píldora en el topbar, barra o riel de luz sobre el lienzo (sin texto, solo color), o ninguno (queda la luz piloto del logo). El color sale SIEMPRE de la paleta de estado, nunca del acento; todo pulso respeta `prefers-reduced-motion` y el estado va también en `title` + `aria-live` (el color nunca es el único canal). → `registry/app/components/SystemBeacon.vue (place="topbar" + place="canvas", both mounted in layouts/default.vue) + app/plugins/beacon-mode.client.ts (publishes data-beacon) + app/plugins/health-beacon.client.ts (publishes data-status); pilot in AppLogo.vue + pharos-components.css (.pharos-pilot)`.
